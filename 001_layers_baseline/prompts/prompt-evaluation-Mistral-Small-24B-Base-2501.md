...
* Optional sidecars (if present):
  ...
  * Raw‑vs‑Norm (window): `output-Mistral-Small-24B-Base-2501-pure-next-token-rawlens-window.csv`
  * Raw‑vs‑Norm (full): `output-Mistral-Small-24B-Base-2501-pure-next-token-rawlens.csv` (augmented with: `js_divergence`, `kl_raw_to_norm_bits`, `l1_prob_diff`, `topk_jaccard_raw_norm@50`.)
...
* Bold **semantic layer** (default = `L_semantic_norm`). If `summary.L_semantic_confirmed` exists or `use_confirmed_semantics=true`, bold the **confirmed** layer and note `confirmed_source` (raw/tuned).
