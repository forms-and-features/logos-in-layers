...
* JSON – `001_layers_baseline/run-latest/output-Qwen3-8B.json` (compact summary; per‑token/per‑layer details live in CSVs).
  Read in particular:
...
* Optional sidecars (if present):
  * Prism: `output-Qwen3-8B-records-prism.csv`, `output-Qwen3-8B-pure-next-token-prism.csv`
  * Tuned‑Lens: `output-Qwen3-8B-records-tuned.csv`, `output-Qwen3-8B-pure-next-token-tuned.csv`
  * Raw‑vs‑Norm (window): `output-Qwen3-8B-pure-next-token-rawlens-window.csv`
  * Raw‑vs‑Norm (full): `output-Qwen3-8B-pure-next-token-rawlens.csv` (augmented with: `js_divergence`, `kl_raw_to_norm_bits`, `l1_prob_diff`, `topk_jaccard_raw_norm@50`.)
  * **Tuned variants**: `output-Qwen3-8B-pure-next-token-tuned-variants.csv` (full tuned / rotation‑only / temperature‑only)
  * **Tuned positions audit**: `output-Qwen3-8B-positions-tuned-audit.csv`
  * **Milestones (quick citation):** `output-Qwen3-8B-milestones.csv` (rows for L_copy/L_copy_soft/L_semantic/L_semantic_confirmed)
  * **Artifact audit (quick scan):** `output-Qwen3-8B-artifact-audit.csv`
