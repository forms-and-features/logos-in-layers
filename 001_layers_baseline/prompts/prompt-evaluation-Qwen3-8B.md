...
* Optional sidecars (if present):
  ...
  * Raw‑vs‑Norm (window): output-Qwen3-8B-pure-next-token-rawlens-window.csv (includes `fact_key`/`fact_index` for each prompt)
  * Raw‑vs‑Norm (full): output-Qwen3-8B-pure-next-token-rawlens.csv (augmented with: `js_divergence`, `kl_raw_to_norm_bits`, `l1_prob_diff`, `topk_jaccard_raw_norm@50`; leading columns include `fact_key`/`fact_index`)
