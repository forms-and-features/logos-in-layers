...
* Optional sidecars (if present):
  ...
  * Raw‑vs‑Norm (window): `output-Qwen2.5-72B-pure-next-token-rawlens-window.csv`
  * Raw‑vs‑Norm (full): `output-Qwen2.5-72B-pure-next-token-rawlens.csv` (augmented with: `js_divergence`, `kl_raw_to_norm_bits`, `l1_prob_diff`, `topk_jaccard_raw_norm@50`.)
