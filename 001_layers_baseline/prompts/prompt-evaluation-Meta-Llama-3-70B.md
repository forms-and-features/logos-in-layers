...
* Optional sidecars (if present):
  ...
  * Raw‑vs‑Norm (window): `output-Meta-Llama-3-70B-pure-next-token-rawlens-window.csv`
  * Raw‑vs‑Norm (full): `output-Meta-Llama-3-70B-pure-next-token-rawlens.csv` (augmented with: `js_divergence`, `kl_raw_to_norm_bits`, `l1_prob_diff`, `topk_jaccard_raw_norm@50`.)
