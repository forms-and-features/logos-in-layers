...
* Optional sidecars (if present):
  ...
  * Raw‑vs‑Norm (window): `output-gemma-2-9b-pure-next-token-rawlens-window.csv`
  * Raw‑vs‑Norm (full): `output-gemma-2-9b-pure-next-token-rawlens.csv` (augmented with: `js_divergence`, `kl_raw_to_norm_bits`, `l1_prob_diff`, `topk_jaccard_raw_norm@50`.)
