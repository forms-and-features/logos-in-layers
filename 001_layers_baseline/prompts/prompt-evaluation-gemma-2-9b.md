...
* JSON – 001_layers_baseline/run-latest/output-gemma-2-9b.json (compact summary; per‑token/per‑layer details live in CSVs).
  Read in particular:
...
* Optional sidecars (if present):
  * Prism: output-gemma-2-9b-records-prism.csv, output-gemma-2-9b-pure-next-token-prism.csv
  * Tuned‑Lens: output-gemma-2-9b-records-tuned.csv, output-gemma-2-9b-pure-next-token-tuned.csv
  * Raw‑vs‑Norm (window): output-gemma-2-9b-pure-next-token-rawlens-window.csv (includes `fact_key`/`fact_index` for each prompt)
  * Raw‑vs‑Norm (full): output-gemma-2-9b-pure-next-token-rawlens.csv (augmented with: `js_divergence`, `kl_raw_to_norm_bits`, `l1_prob_diff`, `topk_jaccard_raw_norm@50`; leading columns include `fact_key`/`fact_index`)
  * **Tuned variants**: output-gemma-2-9b-pure-next-token-tuned-variants.csv (full tuned / rotation‑only / temperature‑only)
  * **Tuned positions audit**: output-gemma-2-9b-positions-tuned-audit.csv
  * **Milestones (quick citation):** output-gemma-2-9b-milestones.csv (rows for L_copy/L_copy_soft/L_semantic/L_semantic_confirmed)
  * **Artifact audit (quick scan):** output-gemma-2-9b-artifact-audit.csv
...
TASK
Write **EVAL** in GitHub‑flavoured Markdown with the sections below **in order**. If a claim cannot be grounded in a quoted CSV/JSON line, omit it. The result of your evaluation must be written to: 001_layers_baseline/run-latest/evaluation-gemma-2-9b.md ; append your text to the file, do not overwrite it.
