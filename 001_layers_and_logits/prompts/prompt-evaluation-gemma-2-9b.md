...
- EVAL output file: 001_layers_and_logits/run-latest/evaluation-gemma-2-9b.md
...
Report summary indices from diagnostics: `first_kl_below_0.5`, `first_kl_below_1.0`, `first_rank_le_1`, `first_rank_le_5`, `first_rank_le_10`. Confirm units for KL/entropy are bits. Check the last-layer `kl_to_final_bits` is ≈ 0; if not (e.g., ≈1 bit observed in some Gemma runs), annotate a final‑lens vs final‑head mismatch (norm/precision) and avoid KL-based conclusions; prefer rank-based statements.
...
first_kl_below_1.0 at layer …, first_kl_below_0.5 at layer …; comment on whether KL decreases with depth and whether it is ≈ 0 at final (or flagged as a mismatch).
